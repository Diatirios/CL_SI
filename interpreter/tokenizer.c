    ...
    while (expression[i] != '\n' && expression[i] != EOF && expression[i]!='\0')
    {
        ...
        switch(expression[i]) // these are for chars and strings
        {
            ...
            case '"':
                ...
                strncpy(scratchString, &expression[stringStart], i - stringStart+1);
            ...
            default:
                ...
                scratchSymbol = (char *) malloc(sizeof(char) * (symbolEnd - symbolStart + 2));
                strncpy(scratchSymbol, &expression[symbolStart], (symbolEnd-symbolStart+1));
