		...
		case cellType:
				...
				if (getFirst(expr) != NULL && getFirst(expr)->type == openType)
				{
					...
					if (operator->type == symbolType)
					{
						...
						else if (strcmp(operator->symbolValue,"hasnext")==0)
						{
								return hasnext(args, env);
						}
						else if (strcmp(operator->symbolValue,"next")==0)
						{
								return next(args, env);
						}
						else if (strcmp(operator->symbolValue,"current")==0)
						{
								return current(args, env);
						}
...
Value *iterator(Value *args, Environment *env)
{
	...
	if (valueList != NULL)
	{
		if (valueList->type == cellType)
		{
			...
			value->iteratorValue->cell = getTail(valueList);
			return value;
		}
	}
	else
...
Value *hasnext(Value *args, Environment *env)
{
	...
	if (getFirst(args))
	{
		value = envLookup(getFirst(args)->symbolValue,env);
		if (value)
		{
			if (value->type == iteratorType)
			{
				...
				boolean->boolValue = (value->iteratorValue->cell->type == cellType 
												&& getTail(value->iteratorValue->cell) != NULL 
												&& getTail(value->iteratorValue->cell)->type == cellType );
				return boolean;
			}
		}
		else
