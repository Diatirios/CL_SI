...
Value* apply(Value* function, Value* actualArgs, Environment* env)
{
    ...
    else if (function->type == closureType)
    {
        ...
        Value *copiedArgs = deepCopy(actualArgs);
        Environment *temp = createFrameWithSize(function->closureValue->parent, 16);
...
Value* evalEach(Value* args, Environment* env)
{
    ...
    Value *copiedArgs = deepCopy(args);
    Value *curArgs = copiedArgs;
    Value *temp;
    Value *head, *paren;
...
//! eval letrec
Value *evalLet(Value *originalArgs, Environment *env)
{
    Value *toCheck;
    ...
    if (count < 2)
    {
        ...
        printValue(originalArgs);
        printf("\n");
        return NULL;
    }
    Value *copiedArgs = deepCopy(originalArgs);
    Value *args = copiedArgs;
...
Value* evalLetStar(Value* originalArgs, Environment* env)
{
    Value *toCheck;
...
Value *evalOr(Value *args, Environment *env)
{
    ...
    else
    {
        ...
        while (getFirst(toCheck))
        {
            ...
            else if (!(toValidate->type == booleanType && (!(toValidate->boolValue))))
            {
                return toValidate;
            }
            toCheck = getTail(toCheck);
...
Value *iterator(Value *args, Environment *env)
{
    ...
    else*/
    {
        ...
        {
            valueList = eval(getFirst(args),env);
			}
        if (valueList != NULL)
        {
        		if (valueList->type == cellType)
            {
               ...
               printf("1: %d\n",getFirst(valueList)->type);
                		printf("2: %d\n",getFirst(getTail(valueList))->type);
                		printf("3: %p\n",getFirst(getTail(getTail(valueList))));
               return value;
            }
        }
        else
...
{
    if (args == NULL||args->cons->car== NULL)
    {
        printf("syntax error: missing components here\n");
        return NULL;
    }
    ...
    {*/
        ...
        Value *value = NULL;
            ...
            if (value)
            {
                if (value->type == iteratorType && value->iteratorValue->cell != NULL && value->iteratorValue->cell->type == cellType)
                {
                	if (getFirst(getTail(value->iteratorValue->cell)) != NULL)
                	{
                		//printf("value->iteratorValue->cell->cons->cdr->cons->car->type %d\n", getFirst(getTail(getTail(value->iteratorValue->cell)))->type);
                		/*printf("1: %d\n",getFirst(value->iteratorValue->cell)->type);
                		printf("2: %d\n",getFirst(getTail(value->iteratorValue->cell))->type);
                		printf("3: %p\n",getFirst(getTail(getTail(value->iteratorValue->cell))));*/
                        value->iteratorValue->cell = getTail(value->iteratorValue->cell);
                	}
    ...
    {*/
        ...
        value = getFirst(args);
            if (value)
            {
                ...
                {
                	if (value->iteratorValue->cell->cons->cdr->cons->car != NULL && value->iteratorValue->cell->cons->cdr->cons->car != NULL)
                    return value->iteratorValue->cell->cons->cdr->cons->car;
                }
            }
            else
            {
